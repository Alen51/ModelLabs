...
{	
	public class NetworkModel
	{
		...
		public List<long> GetGids()
		{
			...
			foreach (var container in networkDataModel.Values)
			{
				gids.AddRange(container.Entities.Keys);
			}
		...
		public List<long> GetReferencedEntities(long globalId, ModelCode association)
		{
			List<long> relatedGids = new List<long>();
			IdentifiedObject io = GetEntity(globalId);
			Property propertyRef = null;

			if (Property.GetPropertyType(association) == PropertyType.Reference)
			{
				propertyRef = io.GetProperty(association);
				long relatedGidFromProperty = propertyRef.AsReference();
			...
			else if (Property.GetPropertyType(association) == PropertyType.ReferenceVector)
			{
