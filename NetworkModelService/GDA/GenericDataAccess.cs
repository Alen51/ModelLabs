...
namespace FTN.Services.NetworkModelService
{
	public class GenericDataAccess : INetworkModelGDAContract
	{
		...
		public List<long> GetGids()
		{
			...
			{
				string message = string.Format("Getting all gids failed. \n{0}", e.Message);
				CommonTrace.WriteTrace(CommonTrace.TraceError, message);
				throw new Exception(message);
			}
		}
		...
		public List<long> GetReferencedEntities(long globalId, ModelCode association)
		{
			try
			{
				return nm.GetReferencedEntities(globalId, association);
			}
			catch (Exception e)
			{
				string message = string.Format("Getting referenced entities failed. \n{0}", e.Message);
				CommonTrace.WriteTrace(CommonTrace.TraceError, message);
				throw new Exception(message);
			}
		}
